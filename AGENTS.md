...
**Filesystem paths are a narrower rule than the one above, because two subcommands print them deliberately.** `doctor` exists to report which paths it resolved, and `import` names the directory it was given. Both are commands a person typed, and their output - stdout or stderr - is that person's terminal.
