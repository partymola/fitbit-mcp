...
Filesystem paths are a narrower rule, because two places print them deliberately. `doctor` exists to tell the user which paths it resolved, and the `import` subcommand names the directory it was given - both are the user's own paths, on their own terminal, in response to a command they typed. What must not carry a path is anything **stored or returned to the model**: `sync_log.notes` and every tool response. That is why `importer` logs a file's name and not its full path.
