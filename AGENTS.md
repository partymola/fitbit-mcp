...
**No message built from an API response reaches a user, a log or `sync_log`.** `api.get` raises a status code and a path, never the body: a Fitbit error body can quote the measurement that caused it, and `run_sync` writes `str(e)` straight into `sync_log.notes`. Pinned by `test_an_error_body_never_reaches_the_message` in `tests/test_api.py`.
